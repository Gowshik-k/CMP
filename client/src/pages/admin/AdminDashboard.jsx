...
const AdminDashboard = ({ user }) => {
    ...
    const sidebarItems = [
        ...
        { id: 'home', label: 'Back to Site', icon: Shield },
    ];
    ...
    return (
        <div className="flex h-screen bg-[#f8fafc]">
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                <header className="h-20 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-8 flex items-center justify-between z-10 sticky top-0">
                    <div>
                        <h1 className="text-xl font-black text-zinc-900 tracking-tight">
                            {sidebarItems.find(i => i.id === activeTab)?.label}
                        </h1>
                        <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-wider mt-0.5">Admin Management Portal</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="bg-zinc-50 p-2 rounded-xl border border-zinc-200 shadow-sm relative cursor-pointer hover:bg-zinc-100 transition-colors group">
                            <div className="w-2 h-2 bg-blue-500 rounded-full absolute top-1.5 right-1.5 border-2 border-white animate-pulse"></div>
                            <Users className="w-5 h-5 text-zinc-600 group-hover:scale-110 transition-transform" />
                        </div>
                        <div className="w-10 h-10 rounded-xl bg-zinc-900 border-2 border-white shadow-sm overflow-hidden flex items-center justify-center font-bold text-white text-sm">
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
                    ...
                    {['conferences', 'submissions', 'settings'].includes(activeTab) && (
                        <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] bg-white rounded-[32px] border border-zinc-200 border-dashed animate-fade-in-up">
                            <div className="w-16 h-16 bg-zinc-50 rounded-2xl flex items-center justify-center text-zinc-300 mb-4 transition-transform hover:scale-110 duration-300">
                                {activeTab === 'conferences' && <Calendar className="w-8 h-8" />}
                                {activeTab === 'submissions' && <FileText className="w-8 h-8" />}
                                {activeTab === 'settings' && <Settings className="w-8 h-8" />}
                            </div>
                            <h3 className="text-xl font-black text-zinc-900 mb-2 capitalize">{activeTab}</h3>
                            <p className="text-zinc-500 text-sm max-w-[320px] text-center font-medium leading-relaxed">
                                This module is currently under development and will be available in a future update. Stay tuned!
                            </p>
                        </div>
                    )}
                </div>
